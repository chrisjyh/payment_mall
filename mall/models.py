...
class Order(models.Model):
    ...
    class Status(models.TextChoices):
        ...
        PAID = "paid", "결제완료",
        PREPARED_PROJECT = "prepared_project", "상품준비중",
        SHIPPED = "shipped", "배송중",
    ...
    total_amount = models.PositiveBigIntegerField("결재금액")
    status = models.CharField(
        "진행상태",
        max_length=20,
    ...
    # redirect 함수코드에서 resolve_url을 내부에서 호출
    # 해당모델에 상세보디 뷰를 생성할때는 해당 함수에서 1번만 url reverse를 하고 활용 
    ...
    @classmethod
    def create_from_cart(cls, user: User, cart_product_qs: QuerySet[CartProduct]) -> "Order":
...
class OrderPayment(AbstractPortonePayment):
    ...
    # AbstractPortonePayment의 update재정의
    def update(self):
        super.update()
